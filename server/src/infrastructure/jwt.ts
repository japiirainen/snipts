...
export const verifyAccessToken = (token: string): E.Either<JwtError, { username: string }> =>
   E.tryCatch(
      () => jwt.verify(token, config.jwt.at!) as never,
      e => (e instanceof Error ? new JwtError(e.message) : new JwtError('unknown error'))
   )

export const verifyRefreshToken = (token: string): E.Either<JwtError, { username: string }> =>
   E.tryCatch(
